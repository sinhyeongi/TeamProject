...
※ 카카오톡 : 카카오톡 내 플러스 친구 > 여기어때 고객행복센터 추가','qa_예약');
...
insert into boarder(id,title,content,category) values('admin',
'포인트는 무엇인가요?',
'리얼리뷰 작성이나 이벤트 참여 등 여기어때 서비스를 이용하면서 적립된 포인트를 말해요.
포인트는 사용 기간 내에 여기어때 서비스를 이용할 때 쓸 수 있어요.','qa_포인트');
...
점검 시간 동안에는 여기어때 서비스 이용이 일시 중지됩니다.
