...
[접수방법]
※ 이메일 : help@goodchoice.kr (예약자명, 숙소명, 체크인 날짜 필수)
※ 카카오톡 : 카카오톡 내 플러스 친구 > 자바 고객행복센터 추가','qa_예약');

insert into boarder(id,title,content,category) values('admin',
'계좌 등록 후 은행에서 문자가 왔어요.',
'간편계좌이체, TOSS에 계좌 등록 시 은행에서 자동이체 서비스가 등록되었다는 문자가 발송될 수 있습니다.

매달 일정 금액이 자동이체 되는 것이 아니라, 결제 시 비밀번호 인증 후 이체되는 서비스이니 안심하고 이용해주시길 바랍니다.','qa_결제');

insert into boarder(id,title,content,category) values('admin',
'결제 영수증 발급받고 싶어요.',
'결제 영수증은 결제수단에 따라 확인/발급 방법이 상이하오니 아래 내용을 참고해 주시기 바랍니다.

[신용/체크카드, KB pay, 법인카드, 휴대폰결제, TOSS]
- 자바 앱 > 하단 내정보 > 예약/구매내역 > 결제 증빙 보기
...
- 단, 19년 12월 26일 10시 이전 결제 건은 자바 고객행복센터로 요청

[간편계좌이체 / TOSS / PAYCO]
- 결제 단계에서 현금영수증 신청하면 자동으로 발행
- 단, 간편계좌이체의 경우 신청 누락 시 자바 고객행복센터로 요청

[카카오페이] 
- 카카오페이머니 결제 시 자동으로 발행
- 자진발급 : 카카오톡 > pay > 우측톱니바퀴(나의 카카오페이) > 이용내역 > 결제 > 개별 상세내역 > 현금영수증 확인','qa_결제');

insert into boarder(id,title,content,category) values('admin',
'포인트는 무엇인가요?',
'리얼리뷰 작성이나 이벤트 참여 등 자바 서비스를 이용하면서 적립된 포인트를 말해요.
포인트는 사용 기간 내에 자바 서비스를 이용할 때 쓸 수 있어요.','qa_포인트');

insert into boarder(id,title,content,category) values('admin',
'포인트 사용 기간이 궁금해요',
'포인트 사용 기간은 적립일에 따라 각각 달라요.','qa_포인트');

insert into boarder(id,title,content,category) values('admin',
'쿠폰함에서 쿠폰이 사라졌어요.',
'사용하지 않은 쿠폰은 유효기간이 지나면 쿠폰함에서 자동 삭제가 됩니다.
 
앱 하단 내정보 > 쿠폰함에서 유효기간을 미리 확인해 주세요.','qa_쿠폰');

insert into boarder(id,title,content,category) values('admin',
'예약 시 사용가능한 쿠폰이 없어요.',
'쿠폰마다 사용 가능한 숙소/요일/객실타입/금액대 등이 다르게 적용되며, 유효기간 및 사용 기준이 달라집니다. 
 
앱 하단 내정보 > 쿠폰함에서 보유한 쿠폰의 사용 기준을 확인해 주세요.','qa_쿠폰');

# 공지사항 더미
insert into boarder(id,title,content,category) values('admin','1:1 문의 서비스 종료 안내','﻿안녕하세요. 자바입니다.

더 나은 서비스 제공을 위해 1:1문의 서비스가 종료될 예정입니다.
너그러운 마음으로 양해 부탁드리며, 자세한 변경 사항은 아래 내용을 확인해 주시기 바랍니다.

■ 서비스 종료일 : 2024년 03월 11일(월)
※ 앱버전 5.31.0 이상에서 1:1 문의 서비스 종료

지금까지 1:1 문의 서비스를이용해 주셔서 감사드리며,
종료 이후에는 고객행복센터(1670-6250) 또는 1:1 카카오 채팅상담으로 문의해 주시기 바랍니다.

앞으로 더 좋은 서비스를 제공해드릴 수 있도록 노력하겠습니다. 

감사합니다.
자바 드림.','notice_공지');
insert into boarder(id,title,content,category) values('admin','서비스 점검 안내(3/5 03:30 ~ 06:00)','안녕하세요. 자바입니다.

더 나은 서비스 제공을 위한 점검 안내드립니다.
점검 시간 동안에는 자바 서비스 이용이 일시 중지됩니다.

■ 자바 시스템 점검 안내
1. 점검 일정:
- 2024년 3월 5일(화) 오전 3시 30분 ~ 6시 00분 (2시간 30분)

2. 점검 사유:
- 시스템 업그레이드

3. 점검 영향:
- 점검 시간 동안 자바 서비스 이용 불가
...
자바 개인정보처리방침이 아래와 같이 변경되어 안내드립니다.

1. 주요 변경 사항
1) 개인정보 제3자 제공 관련 내용 현행화
2) 개인정보 처리위탁 업체 및 재수탁자 현행화
3) PCI-DSS 인증 관련 내용 반영
